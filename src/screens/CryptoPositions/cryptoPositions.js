...
const CryptoPositions = () => {
    ...
    useEffect(() => {
        axios.get(`http://${BACKEND_URL}/api/cryptoPositions/getCryptoPositions`, {
            params: {
                            ...
                            <tbody>
                                {
                                    positions.map((position, key) => {
                                        return (
                                            ...
                                            <tr>
                                                <td id={`${position.instrument + "-" + position.product}`}>{position.instrument}</td>
                                                <td>{position.product}</td>
                                                <td id={`${position.instrument + "-" + position.product}-net-qty`}>{position.net_qty.toFixed(2)}</td>
                                                <td>{position.avg.toFixed(2)}</td>
                                                <td id={`${position.instrument + "-" + position.product}-ltp`}>{0}</td>
                                                <td id={`${position.instrument + "-" + position.product}-buy-value`}>{position.buy_value}</td>
                                                <td id={`${position.instrument + "-" + position.product}-sell-value`}>{position.sell_value}</td>
                                                <td id={`${position.instrument + "-" + position.product}-current-value`}>{0}</td>
                                                <td id={`${position.instrument + "-" + position.product}-pnl`}>{0}</td>
                                            </tr>
                                        )
                                    })
                                }
